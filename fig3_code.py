...
# ---------------------------------------------------------------------------
obs_file   = 'path/to/inat_physalia_obs_usec_2017-2024.csv'  # pre-filtered East Coast iNaturalist observation CSV
...
def main():
    ...
    #df = pd.concat(
        #[deduplicate_group(g) for _, g in df.groupby(df["observed_on"].dt.date)]
    ...
    for ax, region in zip(axes3, regions):
        ...
        ax.text(0.985, 0.96, f'N={n_total}', transform=ax.transAxes,
        ...
        elif region == "Georgia to Cape Hatteras":
            ymax = ax.get_ylim()[1]
            ax.set_yticks(np.arange(0, ymax + 25, 25))
        elif region == "North of Cape Hatteras":
            ymax = ax.get_ylim()[1]
            ax.set_yticks(np.arange(0, ymax + 15, 15))

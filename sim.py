...
def run_simulation(fieldset, offshore_df, trial_number, output_directory,
    ...
    beached_filename = f"beached_10000_run_{trial_number}_adaptive_smag.csv"
    output_filename = f"10000_run_{trial_number}_adaptive_smag.zarr"
    os.makedirs(output_directory, exist_ok=True)
    full_output_path = os.path.join(output_directory, output_filename)
    beached_output_path = os.path.join(output_directory, beached_filename)

    # Function to write beached particles to a CSV file
    def write_beached_to_csv(beached, filename):
    ...
    def SmagorinskyKernel(particle, fieldset, time):
        ...
        if oob_smag:
            particle.delete()
        elif particle.beached == 0 and time >= particle.start_times:
            dx = 1.0 / 12.0
    ...
    def PhysaliaAdvectionKernel(particle, fieldset, time):
        ...
        if oob_pa:
            ...
            if particle.beached == 0 and time >= particle.start_times:
    ...
    # Permanently stop the first time-step a beached particle is detected
    def beached_check(particle, fieldset, time):
        ...
        if oob_bc:
            ...
            if particle.beached == 1 and particle.stopped == 0:
                particle.stopped = 1

    # Combine all kernels
    combined_kernel = pset.Kernel(handle_out_of_bounds) + pset.Kernel(SmagorinskyKernel) + pset.Kernel(PhysaliaAdvectionKernel) + pset.Kernel(land_check) + pset.Kernel(beached_check)
    ...
    # Extract beached particle info from zarr (particle.stopped flips 0→1 at first beaching)
    ...
    for traj in range(stopped.shape[0]):
        ...
        if len(idx) > 0:
            i = idx[0]
            beached_records.append((lat_out[traj, i], lon_out[traj, i], time_out[traj, i], hand_out[traj, i]))
    ds_out.close()
    with open(beached_output_path, 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Latitude', 'Longitude', 'time', 'handedness'])
        for item in beached_records:
            writer.writerow(item)
...
def main():
    ...
    land_mask    = 'path/to/land_mask.tif'                   # land mask raster (GeoTIFF)
    base_data    = 'path/to/base_data.nc'                   # base U/V fields (NetCDF)
    current_file = 'path/to/ocean_currents_6hr.nc'          # 6-hourly merged ocean currents (NetCDF)
    wind_file    = 'path/to/wind_6hr.nc'                    # 6-hourly wind fields (NetCDF)
    output_dir   = 'output/simulation_runs'                 # directory where zarr + beached CSVs are written
    # -----------------------------------------------------------------------
